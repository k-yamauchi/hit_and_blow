    ...
    for agent_name in agents:
        ...
        for game in range(games):
            ...
            while not done and turn < max_turns:
                ...
                agent_history = convert_history_format(env.history)
...
def visualize_results(results):
    ...
    for agent_name, stats in results.items():
        # ターン数ごとの頻度をカウント
        turns_count = np.bincount(stats['turns_history'], minlength=11)
        total_games = stats['games']
        
        # 頻度を割合に変換
        turns_percentage = (turns_count / total_games) * 100
        
        # プロット
        plt.bar(
            np.arange(len(turns_percentage)) + 0.1 * list(results.keys()).index(agent_name),
            turns_percentage,
            width=0.1,
            label=agent_name,
            alpha=0.7
