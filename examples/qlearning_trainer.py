...
class QLearningTrainer:
    ...
    def __init__(self, digits=3, number_range=6, allow_repetition=False):
        ...
        # 相手エージェント用のタイプ
        self.opponent_types = ['rule_based', 'probabilistic']
    ...
    def train(self, num_episodes=1000, batch_size=32, decay_exploration=True):
        ...
        for episode in range(num_episodes):
            ...
            if decay_exploration:
                self.agent.exploration_rate = initial_exploration_rate * (1 - episode / num_episodes)
    ...
    def _run_training_episode(self, game, opponent_type):
        ...
        # 勝敗による報酬
        final_reward = 10.0 if winner == 1 else -5.0 if winner == 2 else 0.0
    ...
    def evaluate(self, num_games=100, opponent_types=None):
        ...
        if opponent_types is None:
            opponent_types = ['rule_based', 'probabilistic', 'hybrid_q_learning']
    ...
    def save_model(self, path="models/q_learning_model.pkl"):
        """モデルを保存"""
    ...
    def load_model(self, path="models/q_learning_model.pkl"):
        """モデルを読み込み"""
        success = self.agent.load_model(path)
...
def main():
    parser = argparse.ArgumentParser(description='Hit and Blow Q学習トレーナー')
    ...
    parser.add_argument('--save-model', type=str, default='models/q_learning_model.pkl', 
                      help='モデルの保存パス')
    parser.add_argument('--load-model', type=str, 
                      help='読み込むモデルのパス（指定しない場合は新規学習）')
